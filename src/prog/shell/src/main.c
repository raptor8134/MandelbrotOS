...
void main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    fgets(
      cmd_line, LINE_LEN,
      stdin); // Reads in the user's command from stdin and save it in 'args'.
    // printf("### MARCO 01\n");
    ...
    while (counter < number_of_cmds) {
      ...
      argv = parse_args(
        cmds[counter]); // Parse 'args' (mini-shell's argument) into a 'argv'.

      if (wasTerminated(argv[0])) {
        exit(0);
      }
      ...
      if (pathName != NULL) {
        // ### The calling process can access a file pathname in the environment
        // list.

        int saved_stdi = set_redirectedIput(iput);
        int saved_stdo = set_redirectedOput(oput);
        if (saved_stdi == -2 || saved_stdo == -2)
          break;
        ...
        // ### The calling process cannot access a file pathname in the
        // environment list.
        perror(0);
      }
