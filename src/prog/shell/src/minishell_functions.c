...
/**
 * Returns 0 if the cmd is supposed to run on the background. Returns -1
 * Otherwise.
 */
 ...
 * Otherwise, Returns -1 if Redirect Input is not required or -2 if there is an
 * error throughout this function.
 */
 ...
 * Otherwise, Returns -1 if Redirect Output is not required or -2 if there is an
 * error throughout this function.
 */
...
char *get_redirectedIput(char *cmd_line) {
  ...
  if (ptrLstChar != NULL) {
    *ptrLstChar = '\0';
    ...
    while (*ptrFstChar == ' ')
      ptrFstChar += 1; // Removes Leading Whitespace.
    while (*(ptrLstChar - 1) == ' ')
      ptrLstChar -= 1; // Removes Trailing Whitespace.
...
char **get_cmds(char *cmd_line, int *number_of_cmds) {
  ...
  while (ptrLstChar != NULL) {
    ...
    cmds[*number_of_cmds][ptrLstChar - ptrFstChar] =
      '\0'; // Inserts '\0' Terminator.
...
char **parse_args(char *args) {
  ...
  while (ptrEndCh != NULL && numberOfArgs < MAX_ARGS) {
    if (ptrBeginCh != ptrEndCh) {
      argv[numberOfArgs] =
        (char *)malloc(MAX_ARGS_LEN + 1); // +1: Terminating Character.
      strncpy(argv[numberOfArgs], ptrBeginCh, ptrEndCh - ptrBeginCh);
      argv[numberOfArgs][ptrEndCh - ptrBeginCh] =
        '\0'; // Inserts '\0' Terminator.
  ...
  if (numberOfArgs < MAX_ARGS) {
    ...
    ptrEndCh = ptrBeginCh + strlen(ptrBeginCh);
...
/** Checks whether the calling process can access a file pathname in the
 * environment list. Returns the file pathname in the environment list of the
 * calling process if the latter can access the former. Otherwise, returns null
 * pointer.
 */
char *get_pathName(char *paths, char *cmd) {
  char *pathName;
  pathName = (char *)malloc(MAX_PATHS_LEN + MAX_ARGS_LEN +
                            1); // +1: Terminating Character.
